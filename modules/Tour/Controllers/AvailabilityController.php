...
class AvailabilityController extends FrontendController
{
    ...
    public function get_disabled_dates(Request $request, $tourId)
    {

      $tour = Tour::where('slug', $tourId)->first();
    ...
    public function all_menus(Request $request, $tour_id)
    {
    ...
    public function disable_time_slot(Request $request)
    {
        $this->checkPermission('tour_create');
        $q = $this->tourClass::query();
        if ($request->query('s')) {
        ...
        return view('Tour::frontend.user.disable_time_slot', compact('rows', 'breadcrumbs', 'current_month', 'page_title', 'request'));
    }
    public function getEnabledTimeSlots(Request $request)
    {
        $day = $request->start_date;
        $tour_id = $request->target_id;
    ...
    public function Store_disable_time_slot(Request $request)
    {
        $request->validate([
            'target_id'  => 'required',
        ]);
        ...
        $time_slot = CategoryTimeSlot::find($request->input('time_slot_id'));
        $newDate = new Bookning_dates();
        $newDate->tour_id = $request->input('target_id');
        $newDate->booking_id = 1;
        $newDate->category_id = $tour->category_id;
        $newDate->day = $request->input('day');
        $newDate->start_from = $time_slot->start_at;
        $newDate->end_at = $time_slot->end_at;
        $newDate->active = 1;
        $newDate->save();
        return $this->sendSuccess([], __("Update Success"));
    }
