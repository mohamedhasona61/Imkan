...
class Tour extends Bookable
{
    ...
    public    $checkout_booking_detail_file_web       = 'Tour::frontend/booking/detail_web';
    ...
    public    $email_new_booking_file_app             = 'Tour::emails.new_booking_detail_app'; 
    public    $edit_booking             = 'Tour::emails.new_booking_detail_app'; 
    public    $type                               = 'tour';
    ...
    public function saveMeta(\Illuminate\Http\Request $request)
    {
        ...
        $arg = $request->input();
        $openHours = [
            "1" => ["enable" => "1", "from" => "10:00", "to" => "12:00"],
            "2" => ["enable" => "1", "from" => "12:00", "to" => "02:00"],
            "3" => ["enable" => "1", "from" => "12:00", "to" => "02:00"],
            "4" => ["enable" => "1", "from" => "12:00", "to" => "02:00"],
            "5" => ["enable" => "1", "from" => "12:00", "to" => "02:00"],
            "6" => ["enable" => "1", "from" => "12:00", "to" => "02:00"],
            "7" => ["enable" => "1", "from" => "12:00", "to" => "02:00"],
        ];

        $arg['open_hours'] = $openHours;
    ...
    public function addToCart(Request $request)
    {
        ...
        $selected_menus = $request->input('selectedItems') ?? "'[ ]'";
        if (!empty($selected_menus) && is_array($selected_menus)) {
        } else {
        }
        $start_date = new \DateTime($request->input('start_date'));
        $dayOfWeek = $start_date->format('l');
            ...
            if ($meta) {
                ...
                if ($meta->enable_person_types and !empty($meta->person_types)) {
                    if ($dayOfWeek === 'Thursday' || $dayOfWeek === 'Friday') {
                        $total = $person_types_input[0]['number'] * $person_types_input[0]['special_price'];
                        $total_guests = $person_types_input[0]['number'];
                    } else {
                        $total = $person_types_input[0]['number'] * $person_types_input[0]['price'];
        ...
        $tour = Tour::find($request->input('service_id'));
        ...
        if (!empty($this->enable_service_fee) and !empty($list_service_fee = $this->service_fee)) {
            $total_service_fee = $this->calculateServiceFees($list_service_fee, $total, $total_guests);
        ...
        $booking->total_before_discount = $total;
        $booking->is_app = 1;
        ...
        if ($check) {
            ...
            $times = explode(" - ", $timeSlot[0]);
            $startTime = $times[0];
            $endTime = $times[1];
            $category_booking = Tour::find($booking->object_id);
            

            $dates = new Bookning_dates();

            $dates->booking_id = $booking->id;

            $dates->tour_id = $booking->object_id;

            $dates->category_id = $category_booking->category_id;
            $dates->day = $booking->start_date;
            $dates->active = 0;
            $dates->start_from = $startTime;
            $dates->end_at = $endTime;
            $dates->save();


            $this->bookingClass::clearDraftBookings();
            $booking->addMeta('duration', $this->duration);
            $booking->addMeta('base_price', $booking->total_guests * $person_types_input[0]['display_price']);
            $booking->addMeta('guests', max($total_guests, $request->input('guests')));
            $booking->addMeta('extra_price', $extra_price);
            $booking->addMeta('person_types', $person_types_input);
            $booking->addMeta('discount_by_people', $discount_by_people);
            $booking->addMeta('menus', $selected_menus);
            if ($this->isFixedDate()) {
            ...
            foreach ($extra_price as $extra_time) {
                if ($extra_time['type'] == "per_hour") {
                    ...
                    $booking_date->tour_id = $category_booking->id;
                    $booking_date->category_id = $category_booking->category_id;
                    $booking_date->booking_id = $booking->id;
                    $booking_date->start_from = $last_booking_dates->end_at;
                    $booking_date->end_at = Carbon::parse($last_booking_dates->end_at)->addHours(2);
