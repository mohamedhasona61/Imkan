...
class TourController extends AdminController
{
    ...
    public function store(Request $request, $id)
    {
        ...
        $row->menu_id = json_encode($menuIds);
        $selected = 0;
        $row->select_price = $selected;

        $res = $row->saveOriginOrTranslation($request->input('lang'), true);
        if ($id > 0) {
            $time = CategoryTimeSlot::where('tour_id', $row->id)->get();
            if ($time->count() > 0) {
                foreach ($time as $tm) {
                    $tm->delete();
                }
                ...
                if ($row->category_id == 14) {
                    $startAt = "10:00:00";
                    $startTime = Carbon::createFromFormat('H:i:s', $startAt);
                    for ($i = 0; $i < 8; $i++) {
                        $endTime = $startTime->addHour(2);
                        $endAt = $endTime->format('H:i:s');
                        $time_slot = new CategoryTimeSlot();
                        $time_slot->tour_id = $row->id;
                        $time_slot->category_id = $row->category_id;
                        $time_slot->start_at = $startAt;
                        $time_slot->end_at = $endAt;
                        $time_slot->active = 0;
                        $time_slot->save();
                    ...
                    if ($request->open_hours) {
                        foreach ($request->open_hours as $open_hours) {
                            if (isset($open_hours['enable']) && $open_hours['enable'] == 1) {
                                $time_slot = new CategoryTimeSlot();
                                $time_slot->tour_id = $row->id;
                                $time_slot->category_id = $row->category_id;

                                $time_slot->start_at = $open_hours['from'];
                                $time_slot->end_at = $open_hours['to'];
                                $time_slot->active = 0;
                                $time_slot->save();
                            }
                        }
