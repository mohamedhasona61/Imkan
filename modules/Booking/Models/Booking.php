...
class Booking extends BaseModel
{
    ...
    public static function getBookingHistory($booking_status = false, $customer_id = false, $vendor_id = false, $service = false , $category_id=false)
    {
        ...
        if (!empty($category_id)) {
            $list_booking->where("category_id", $category_id);
        }
        if (!empty($vendor_id)) {
        ...
        $list_booking->where('status', '!=', 'draft')->where('status', '!=', 'pending');
    ...
    public static function getBookingInRanges($object_id, $object_model, $from, $to, $object_child_id = false)
    {
        ...
        $query = parent::selectRaw(" * , SUM( total_guests ) as total_guests ")->where(['object_id' => $object_id, 'object_model' => $object_model,])->whereNotIn('status', static::$notAcceptedStatus)
            ->where('end_date', '>=', $from)
            ->where('start_date', '<=', $to)
            ->groupBy('start_date')
            ->take(200);
