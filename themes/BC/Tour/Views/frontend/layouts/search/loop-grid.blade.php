...
@endphp
<div class="item-tour {{ $wrap_class ?? '' }} row ">
        ...
        <a @if (!empty($blank)) target="_blank" @endif
            ...
            @if ($row->image_url)
                @if (!empty($disable_lazyload))
                    <img style="object-fit: contain !important;width: 100%; height: unset;" src="{{ $row->image_url }}" class="img-responsive" alt="{{ $location->name ?? '' }}">
                @else
